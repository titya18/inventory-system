...
import { getStockReturnById } from "@/api/stockReturn";
...
const ReportReturn: React.FC = () => {
    ...
    const exportData = returnData.map((row, index) => ({
        "No": (page - 1) * pageSize + index + 1,
        "Reference": row.ref,
        "Return Date": row.returnDate ? format(new Date(row.returnDate), "dd-MMM-yyyy") : "",
        "Return By": fullName(row.returner),
    ...
    const handleViewDetail = async (id: number) => {
        try {
            ...
            const detail = await getStockReturnById(id);
            setSelectedReturn(detail);
        } catch (error: any) {
            toast.error(error.message || "Failed to fetch stock return detail");
                                                    ...
                                                    returnData.map((row, index) => (
                                                        <tr key={index}>
                                                            ...
                                                            {visibleCols.includes("Return Date") && (
                                                                <td>{row.returnDate ? format(new Date(row.returnDate), "dd-MMM-yyyy") : "N/A"}</td>
                                                            )}

                                                            {visibleCols.includes("Return By") && (
                                                                <td>{fullName(row.returner)}</td>
                                                            )}
                                ...
                                {detailLoading ? (
                                    ...
                                    <>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
                                            <div>
                                                <strong>Reference:</strong> {selectedReturn.ref}
                                            </div>
                                            <div>
                                                <strong>Return Date:</strong>{" "}
                                                {selectedReturn.returnDate
                                                    ? format(new Date(selectedReturn.returnDate), "dd-MMM-yyyy")
                                                    : "N/A"}
                                            </div>
                                            <div>
                                                <strong>Status:</strong> {selectedReturn.StatusType}
                                            </div>
                                            <div>
                                                <strong>Branch:</strong> {selectedReturn.branch?.name || "N/A"}
                                            </div>
                                            <div>
                                                <strong>Return By:</strong> {fullName(selectedReturn.returner)}
                                            </div>
                                            <div>
                                                <strong>Total Lines:</strong> {selectedReturn.returnDetails?.length || 0}
                                            </div>
                                        </div>

                                        <div className="mb-4">
                                            <strong>Note:</strong> {selectedReturn.note || "N/A"}
                                        </div>
                                        ...
                                        <div className="overflow-x-auto">
                                            <table className="table-auto w-full border-collapse border border-gray-200">
                                                ...
                                                <tbody>
                                                    {selectedReturn.returnDetails?.length > 0 ? (
                                                        selectedReturn.returnDetails.map((detail: any, idx: number) => (
                                                            <tr key={detail.id}>
