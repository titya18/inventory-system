...
export const stockValuationReport = async (req: Request, res: Response) => {
  try {
    ...
    const branchCondition = branchFilter?.branchId
      ? `AND s."branchId" = ${Number(branchFilter.branchId)}`
      : "";
    ...
    const rows: any[] = await prisma.$queryRawUnsafe(
      ...
      /** LAST KNOWN UNIT COST BY VARIANT + BRANCH */
      LEFT JOIN LATERAL (
        SELECT sm."unitCost"
        FROM "StockMovements" sm
        WHERE sm."productVariantId" = pv.id
          AND sm."branchId" = s."branchId"
    ...
    /** FULL SUMMARY FOR ALL MATCHING ROWS */
    const summaryResult: any[] = await prisma.$queryRawUnsafe(
      `
      SELECT
        COALESCE(SUM(x."stockValue"), 0)::FLOAT AS "totalStockValue"
      FROM (
        SELECT
          (SUM(s.quantity) * COALESCE(lastCost."unitCost", 0))::FLOAT AS "stockValue"
        FROM "Stocks" s
        ...
        GROUP BY pv.id, s."branchId", lastCost."unitCost"
      ) x
      `,
      likeTerm
    );

    const variantIds = rows.map((r) => Number(r.variantId)).filter(Boolean);
    ...
    if (variantIds.length > 0) {
      ...
      attrs.forEach((a) => {
        if (!attributeMap[a.variantId]) {
          attributeMap[a.variantId] = [];
        }
