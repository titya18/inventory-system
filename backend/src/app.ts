...
// User-uploaded images (may be overridden by a Docker volume mount on /app/public/images)
const publicPath = path.join(process.cwd(), 'public');
app.use('/images', express.static(path.join(publicPath, 'images')));

// Fallback: serve noimage.png from the app bundle (outside the upload path so volume mounts can't hide it)
// In compiled production: __dirname = /app/dist/src  →  ../../public/noimage.png = /app/public/noimage.png
// In dev (ts-node):       __dirname = /app/src        →  ../public/noimage.png   = /app/public/noimage.png
const noimageFile = path.resolve(
  __dirname,
  process.env.NODE_ENV === 'production' ? '../../public/noimage.png' : '../public/noimage.png'
);
app.get('/images/products/noimage.png', (_req, res) => {
...
app.use('/api/cashsession', cashSessionRoute);
