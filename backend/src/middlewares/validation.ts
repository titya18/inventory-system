...
export const validateUserRequest = [
    ...
    body("branchId")
        .if((_value, { req }) => req.body.roleType === "USER") // Only required if roleType is USER
        .notEmpty()
...
export const validateCategoryRequest = [
    ...
    body("code").notEmpty().withMessage("Code must be required"),
    handleValidationErrors
];

export const validateCustomerRequest = [
    body("name").notEmpty().withMessage("Customer's name must be required"),
    body("phone").notEmpty().withMessage("Phone number must be required"),
    handleValidationErrors
];
...
export const validateServiceRequest = [
    body("serviceCode").notEmpty().withMessage("Service code must be required"),
    body("name").notEmpty().withMessage("Name must be required"),
...
export const validateBrandRequest = [
    body("en_name").notEmpty().withMessage("Brand's name is required"),
    handleValidationErrors
];
...
export const validatePurchaseRequest = [
    ...
    body("purchaseDate")
        .customSanitizer((value) => Array.isArray(value) ? value[0] : value)
        .notEmpty()
        .withMessage("Purchase date must be required"),
    // Conditional validation for branchId based on roleType
    body("branchId")
        .if((_value, { req }) => req.body.roleType === "USER") // Only required if roleType is USER
        .notEmpty()
